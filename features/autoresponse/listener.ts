...
import { env, getPrisma } from '../../utils/index.js';

const COMMUNITY_LOGS_CHANNEL = env.NODE_ENV === 'development' ? env.DEV_CHANNEL : 'C085UEFDW6R';
...
export default async function listener({
    ...
    const cooldown = await prisma.autoresponseCooldown.findUnique({
        where: { userId: event.user },
    });
    if (cooldown && Date.now() - cooldown.lastSentAt.getTime() < COOLDOWN_MS) return;
...
We understand that it's hard to trust what you can't see. But giving too much information about ban reasons can and does hurt Hack Clubbers.`;
    ...
    await prisma.autoresponseCooldown.upsert({
        where: { userId: event.user },
        create: { userId: event.user, lastSentAt: new Date() },
