...
import { isSearchAvailable } from './search.js';
import { collectTargets, runNuke } from './run.js';
...
function statusView(title: string, text: string): View {
    return {
        type: 'modal',
        callback_id: 'nuke_status_modal',
...
function registerModal(app: App) {
    app.view('nuke_modal', async ({ view, ack, body, client }) => {
        ...
        if (!isSearchAvailable()) {
            await ack({
                ...
                errors: {
                    nuke_window_block:
                        'Message search is unavailable — SLACK_USER_TOKEN or the browser token is not configured.',
                },
            });
            return;
        }
        ...
        await ack({
            ...
            view: statusView(
                'Searching',
                `:spin-loading: Searching for messages from <@${target}> (${selected.label})…`
            ),
        });
        ...
        let targets;
        try {
            targets = await collectTargets(target, since);
        } catch (e) {
            console.error('[nuke] search failed:', e);
            await client.views.update({
                view_id: viewId,
                view: statusView(
                    'Search failed',
                    ':x: Could not search for messages. Check the logs — the user or browser token may need refreshing.'
                ),
            });
            return;
        }
        ...
        if (total === 0) {
            await client.views.update({
                ...
                view: statusView(
                    'Nothing to delete',
                    `No messages from <@${target}> found in the last ${selected.label.toLowerCase().replace('last ', '')}.`
                ),
            });
            return;
        }
            ...
            blocks: [
                { type: 'section', text: { type: 'mrkdwn', text: lines.join('\n') } },
                {
                    ...
                    elements: [
                        {
                            type: 'mrkdwn',
                            text: 'Only messages visible to the moderation account are found. Deleting a thread parent leaves other people’s replies in place.',
                        },
                    ],
                },
            ],
        ...
        await client.views.update({ view_id: viewId, view: confirmView });
    });
    ...
    app.view('nuke_confirm_modal', async ({ view, ack, body }) => {
        ...
        try {
            ...
            console.error('[nuke] search failed on confirm:', e);
            await postEphemeral(
                command_channel,
                admin,
                ':x: Could not search for messages to delete. Check the logs.'
            );
            return;
        }
