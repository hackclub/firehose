...
async function cleanupChannel(args) {
  ...
  console.log(
    "Checking if user is allowed in channel:",
    channel,
    "User:",
    user,
  );
  ...
  console.log(
    "Channel is read-only, checking message timestamps:",
    ts,
    thread_ts,
  );
  console.log(
    "Allowlist status:",
    allowlist ? "User is allowed" : "User is not allowed",
  );
  ...
  if (thread_ts) {
    ...
    console.log(
      "Message is not in a thread, moving to delete it if it's not allowed",
    );
  }
  ...
  if (text) {
    console.log(
      "User is not allowed in this channel, proceeding to delete message:",
      ts,
    );
